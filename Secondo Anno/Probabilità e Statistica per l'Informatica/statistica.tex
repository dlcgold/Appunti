...
\include{ipotesiParametriche}%Chapter 6 Parametric Hypotesis
\include{ipotesiNonPara}%Chapter 7 NonParametric Hypotesis
%\include{regressioneLineare}%Chapter 8 Regression Linear
...
Ovvero:
\begin{center}
\includegraphics[scale = 0.7]{img/test.png}
\end{center}
\begin{center}
\includegraphics[scale = 0.7]{img/test2.png}
\end{center}
...
sull’altro
\begin{center}
\includegraphics[scale = 0.7]{img/test3.png}
\end{center}
\begin{center}
\includegraphics[scale = 0.7]{img/test4.png}
\end{center}
\begin{center}
\includegraphics[scale = 0.7]{img/test5.png}
\end{center}
\subsection{Outliers}
Uno dei problemi che si presentano nella pratica è la presenza di osservazioni anomale (dovute ad esempio a errori
di misura). Questi valori anomali, detti outliers, possono provocare un peggioramento nella qualità della
regressione, in quanto i valori dei parametri stimati vengono distorti per tenere conto anche di questi valori errati. Per riconoscere i possibili outliers ed eliminarli dal calcolo un modo è quello di calcolare i\textbf{ Residui Studentizzati}:
\[r_{i}=\frac{e_{i}}{\sqrt{M S E\left(1-\frac{1}{n}+\frac{\left(x_{i}-\overline{x}\right)^{2}}{\sum\left(x_{i}-\overline{x}\right)^{2}}\right)}}\]
con $e_i%$ l'i-esimo residuo, SSE somma quadratica dei residui e MSE scarto quadratico medio dei residui $\frac{\sum e_{i}^{2}}{n-2}$.\\
\textbf{Si dimostra che essi sono distribuiti secondo una t di student e
  se il residuo studentizzato di un’osservazione supera un valore di 4–5 allora l'osservazione è un candidato outlier:}
\begin{center}
\includegraphics[scale = 0.7]{img/test6.png}
\end{center}
Non tutti gli outliers hanno però un ugual effetto di modifica dei parametri della regressione:
per riconoscere quelli che la influenzano effettivamente, si usa una misura detta \textbf{D-Cook}, che idnica il cambio di pendenza se si omette un'osservazione; un sospetto outlier è influente se $D > 1 $.
...
\begin{center}
\includegraphics[scale = 3]{img/matrix.png}
\end{center}
...
\end{document} $\mathrm{a}=\left(a_{0}, a_{1}, \ldots, a_{m}\right)^{T}$ è necessario calcolare la matrice inversa di $\left(\mathrm{x}^{T} \cdot \mathrm{x}\right)$ e per questo occorre che tale prodotto matriciale sia non singolare, ovvero con
