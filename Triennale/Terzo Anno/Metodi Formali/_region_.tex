...
\begin{definizione}
  ...
  \begin{figure}[H]
    ...
    \caption{Esempio con $t1\sim_0 t2$ e $t2\sim_0 t3$ ma $t1\not\sim_0 t3$
      e infatti lo scatto di $t1$ o $t3$ non preclude lo scatto dell'altra}
  \end{figure}
  Si ha quindi che $\sim_0$ è \textbf{simmetrica ma non transitiva}.\\
  Definiamo anche la chiusura riflessiva e transitiva
\end{definizione}
\end{teorema}
...
% LocalWords:  Commoner Hack nfc sottoalbero all Conflict

\message{ !name(metodi.tex) !offset(-6068) }
