			...
			\begin{definizione}
				...
				\begin{esempio}
					...
					\[\phi=(x_1\lor x_2)\land (x_1\lor \neg x_2)\land
						...
						\subsection{Riduzione da 3SAT a IndependentSet}
						...
						\[\phi=(\neg x_1\lor x_2\lor x_3)\land(x_1\lor \neg x_2\lor x_3)\land(\neg
							...
							\begin{definizione}
								...
								dimensione.
							\end{definizione}
							\subsubsection{Riduzione da Vertex-Cover a Set-Cover}
							\begin{definizione}
							...
							\begin{proof}(NP-Complete)
								...
								Collego ogni letterale di ogni clausola con ogni
...
\subsection{Problemi NP-Hard non NP}
						...
						\section{Problemi di ottimizzazione}
							...
							\begin{nota}
								...
								questo costo.
							\end{nota}
							\subsection{$\varepsilon$ - approssimazioni}
							\begin{definizione}
							...
							\[\frac{A(x)}{opt(x)}\geq \varepsilon\Longrightarrow\frac{opt(x)}{A(x)}\leq
								...
								\begin{definizione}
									...
									\[\max\left\{\frac{A(x)}{opt(x)},\frac{opt(x)}{A(x)}\right\}\leq r,\mbox{
										...
										\end{nota}
										\subsubsection{Approssimazione per Vertex Cover}
										\begin{definizione}
				...
				\subsection{Complessità parametrica}
										...
										\end{nota}
										\subsubsection{Vertex-cover parametrico}
								...
								% 		\subsubsection{Bounded search trees}
										Si quindi ricorre ai \textbf{Bounded search trees (\textit{alberi di ricerca
										limitati})}.
										\begin{enumerate}
											\item  Si costruisce quindi un albero binario di profondità $ 'k' $, con
											      il numero di nodi che quindi cresce circa per $O(2^k)$ e che sono una copertura
											      candidata. 
											\item Si prende la radice $ 'r' $ dell'albero e dato l'arco $(u,v)\in E$ ho due figli di
											      $ 'r' $, uno con $ 'u' $ e l’altro con $ 'v' $, costruisco quindi due figli a partire dalla
											      radice. 
											      \begin{nota}
											      	Questo si fa in quanto basta avere o $ 'u' $ o $ 'v' $ nella copertura e quindi
											      ...
											      \begin{center}
											      	...
											      	\end{tikzpicture}
											      \end{center}
											\item  Scelto un arco $(x,v)$ non coperto da $ 'u' $ e estendo il nodo di $ 'u' $
											      collegandolo ad altri due nodi, uno con $\{u, v\}$ e uno con $\{u, x\}$ (fisso
											      $k=3$).
											\item Proseguo poi via via con ulteriori archi che non sono coperti dai vertici
										...
										\begin{definizione}
											Definiamo \textbf{ciclo di Eulero} come un ciclo che \textbf{tocca tutti gli archi
										...
										pratica nel trovare un \textbf{ciclo hamiltoniano} di costo minimo. 
										\begin{nota}
											Ricordiamo
										...
										detto \textbf{TSP asimmetrico}
									    \end{nota}
									 \begin{nota}
									         TSP è un problema \textbf{NP-Hard}.
									 \end{nota}
										\begin{definizione}
										    Sia $w_{xy}$ il peso
										...
										\[w_{uv}\leq w_{uz}+w_{zv}\]
											\begin{center}
												\begin{tikzpicture}[shorten >=1pt,node distance=5cm,on grid,auto]
													\node[state] (q_0) {$ 'u' $};
													\node[state] (q_1) [below right=of q_0] {$ 'v' $};
													\node[state] (q_2) [below left=of q_0] {$ 'z' $};
													
													\path[-]
													(q_0) edge  node {$w_{uv}$} (q_1)
													(q_0) edge  node {$w_{uz}$} (q_2)
													(q_2) edge  node {$w_{zv}$} (q_1)
													; 
												\end{tikzpicture}
											\end{center}
										e quindi $ 'w' $ introduce una semi-metrica, che accettiamo come metrica.
                            ...
                            \paragraph{TSP 2-approssimazione}												      	Esiste un'euristica che si dimostra essere 2-approssimante.\\
