...
\begin{figure}
  ...
  \begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
    \node[state] (q_0) {\small $s$};
    \node[state] (q_1) [right=of q_0] {\small $s'$};
    \path[->]
    (q_0) edge  node {\small $a$} (q_1);
  \end{tikzpicture}
...
\begin{itemize}
  ...
  \begin{esempio}
    ...
    avendo:
    \[\frac{p_1\stackrel{\alpha}{\rightarrow}p_1'}{p_3+p_2+p_1
        \stackrel{\alpha}{\rightarrow}p_1'}\]\\
    \[\frac{p_2\stackrel{\alpha}{\rightarrow}p_2'}{p_1+p_3+p_2
        \stackrel{\beta}{\rightarrow}p_2'}\]\\
...
\begin{esempio}
  ...
  \end{center}
\end{esempio}
\section{Relazioni di equivalenza tra processi CCS}
...
\begin{definizione}[Relazione Binaria]
...
\end{definizione}
\begin{definizione}[Regola di equivalenza]
...
\begin{definizione}[Insieme delle tracce]
...
\end{definizione}
\begin{definizione}[Equivalenza rispetto alle tracce]
...
\begin{center}
    ...
    \node at (3,3) (A) {\small\textbf{Tracce Forte}};
    \node at (0,1) (B) {\small\textbf{Bis Forte}};
    \end{scope}
\end{tikzpicture}
\end{center}
  \begin{nota}
...
\begin{esempio}
  ...
  elementi abbiamo $B_i^R$.\\
  Sia quindi, su
  $A=\{in, out\},\,\,\,\overline{A}=\{\overline{in},\overline{out}\}$, con
  $A$ per azioni di ricezione e $\overline{A}$ per azioni di invio, il buffer ha
  capacità 1:
  \[B_0^1=in\cdot B_1^1\]
  ...
  \end{center}
  \newpage
  Proviamo ora a sincronizzare due buffer a capacità 1:
  \[B_0^1|B_0^1\]
...
\subsubsection{Equivalenza debole rispetto alle tracce}
...
\begin{esempio}
  ...
  \begin{itemize}
    ...
    vincente, avendo che $r_2$ e $k_2$ sono isomorfi. \textbf{Una scelta buona su tre
    possibilità }
    ...
    vincente, avendo che $r_2$ e $k_2$ sono isomorfi.\textbf{ Una scelta buona su due
    possibilità }
...
\begin{itemize}
  ...
  \end{esempio}
  \begin{esempio}
    Presi $q=a\cdot b\cdot Nil$, $r=a\cdot(\tau\cdot p+b\cdot Nil)$ e $s=
  ...
  \end{esempio}
\end{itemize}
 \subsection{Proprietà auspicate di una relazione di equivalenza tra processi CCS}
...
\begin{esempio}
  ...
  equivalenti.
\end{esempio}
\subsection{Regole di congruenza per la Bisimulazione Forte}
...
\[p+Nil\sim^{Bis} p\]
\[p|Nil\sim^{Bis} p\]
\subsection{Relazione di congruenza per la Bisimulazione Debole}
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
$\sim^{Bis}$ &  & $\rightarrow$ &  & $\approx^{Bis}$ \\
             &  &            &  &                 \\
$\downarrow$   &  &            &  & $\downarrow$      \\
             &  &            &  &                 \\
$\sim^T$     &  & $\rightarrow$ &  & $\approx^T$    
\end{tabular}%
}
\end{table}
...
% Possiamo quindi dire che la \textbf{bisimulazione debole è una congruenza
%   rispetto agli operatori del CCS diversi da scelta e ricorsione.}\\
\subsection{Relazione di Congruenza}
