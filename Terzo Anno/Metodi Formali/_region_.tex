...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    $\overbrace{\mathbf{G}\ldots\mathbf{G} }^{\mbox{2 o
        più}}A$. \emph{Dimostrazione nel file delle esercitazioni}
    \item si ha che $A\to \mathbf{F}\,A$ è una
    \emph{tautologia} in quanto se si ha che $(M,t)\Vdash A$ allora
    si ha che $(M,t)\Vdash\mathbf{F} A$ per definizione del connettivo \emph{future} e per
    il significato di \emph{implicazione}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale = 0.4]{img/pltl10.png}
    \end{figure}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale = 0.4]{img/pltl11.png}
    \end{figure}
    \newpage
    \item si ha che $\mathbf{G}\,A$ è \emph{equivalente} a
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale = 0.45]{img/pltl12.png}
      \includegraphics[scale = 0.45]{img/pltl13.png}
    \end{figure}
    \newpage
    ...
    $\mathbf{F}\,\mathbf{G}\, A$  sono \emph{equivalenti}.
    \emph{Dimostrazione nel file delle esercitazioni}
...
Si parla del \textbf{teorema di incompletezza di Godel}. Come punto di partenza
\textit{storico} si parla di Hilbert, che pensava alla matematica come un
sistema puramente formale, introducendo i concetti di \textbf{coerenza e
...
dei numeri. \\
Godel cerca di dimostrare coerenza e completezza dell'aritmetica.
\begin{definizione}
...
\begin{itemize}
  \item \textbf{vere}
  \item \textbf{false}
  \item \textsc{ne vere ne false} (come il secondo esempio)
\end{itemize}
ma una frase come:
\begin{center}
...
I teoremi di Godel parlano di sistemi formali.
\begin{definizione}
...
\end{definizione}
\newpage
Godel si chiede se è possibile dimostrare se la correttezza e la completezza
dell'aritmetica.
\begin{teorema}
  ...
  \end{itemize}
  Godel trovò nell'aritmetica un enunciato di questo tipo
\end{teorema}
...
è infinitamente numerabile. Godel asserisce che:
\begin{definizione}
  ...
  che è iniettiva.\\
  Il numero, detto \textbf{numero di Godel} sarà sempre diverso da quello di una
  formula in quanto ho esponenti pari
\end{definizione}
...
\begin{definizione}
  ...
  numeri di Godel di $\Delta$ e $F$, considerandoli come coppia ordinata di
...
$\vdash Dim(m,n)$ sse $M$ è il numero di Godel di una dimostrazione della
formula il cui numero di Godel è $n$.\\
...
Si ha che $Teor(y)$   
lemma diagonale:
numero associatoa beta ha proprietà alpha, beta afferma di avere la pripruetà
alpha
in aritemica quindi ho fmurle che dicono di se stesse di non essere dimostrabili
quindi l'artiemtica è incompelta
\end{document}
...
% LocalWords:  Form root soddisfacibilità pltl png slide Godel Peano sull
% LocalWords:  autoriferimento iniettiva

\message{ !name(metodi.tex) !offset(-3404) }
