...
\begin{definizione}
  ...
  transizione un numero naturale ($U$ è un \textbf{passo} $M[U>$). Si ha inoltre
  che:
  \begin{itemize}
...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    \item $F_J=F\cap[(S_J\times T_J)\cup(T_J\times S_J)]$, ovvero gli archi sono
...
\begin{definizione}
  ...
  un suo T-invariante non negativo. Allora $\exists M:S\to\mathbb{N}$ tale che
  $M$ è una \textbf{marcatura riproducibile} tramite $\underline{J}\in N$, cioè
  tale che: 
...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    pre-transizioni e le post-transizioni dei posti in $S_I$
    ...
    originale 
    \item $W_I=W_{|F_I|}$, ovvero il peso degli archi è il medesimo che
    avrebbero nel sistema originale
  \end{itemize}
  \newpage
  \begin{esempio}
...
\begin{esempio}
  ...
  diverse considerazioni, sempre considerando $\forall M\in[M_0>$,
...
\begin{teorema}
  ...
  \begin{itemize}
    \item $\Sigma=(N,M_0)$ è \textbf{vivo} sse $N$ è \textbf{strettamente
    ...
    \item $\Sigma=(N,M_0)$ è \textbf{vivo} e \textbf{n-limitato} sse $N$ è
...
\begin{definizione}
  ...
  \begin{figure}[H]
    ...
    \caption{Rappresentazione stilizzata di un \emph{sifone} $R$, infatti la
      ...
      transizione, con l'arco blu toglie una marca che non viene rimessa,
      dall'arco nero, nello stesso posto}   
  \end{figure}
...
\begin{teorema}
  ...
  \[M(R)>0\]
  ovvero ogni sifone proprio deve essere marcato nella marcatura iniziale.\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $T_1$ identifica un \textbf{T-invariante} di $N$
  \end{itemize}
\end{itemize}
Vediamo un esempio:
\begin{esempio}
...
\newpage
\begin{teorema}
  Vediamo il \textbf{teorema della copertura da S-componenti}.\\
...
\begin{teorema}
  ...
  \begin{itemize}
    ...
    sse ogni \textbf{sifone proprio} contiene una \textbf{trappola
      ...
      Commoner/Hack} 
    \item $N$ è \textbf{Well Formed} (ovvero $\exists
...
\begin{definizione}
  ...
  Definiamo anche la \textbf{chiusura riflessiva e transitiva} di $\sim_0$, che
...
transizione. Sia inoltre $\,^\bullet[t]=\cup_{t'\in [t]}\,^\bullet t'$, ovvero
...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    \item $\forall s\in \,^\bullet [t],\,\forall t'\in [t]:\,(s,t')\in F$,
...
\begin{definizione}
  ...
  \begin{enumerate}
    ...
    $\forall \,t''=t,\ldots,t'$, vale $(M,t'')\Vdash B$
...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      $(M,t')\Vdash B$, quindi $\exists\,\,t''$ tale che $t\leq t''< t$ tale che
      che $(M,t'')\nVdash A$
      \begin{figure}[H]
        \centering
...
\begin{esempio}
  ...
  è soddisfacibile anche se ogni coppia di formule rappresenta una
  contraddizione.\\
...
\begin{esempio}
  ...
  e, di conseguenza, anche $\neg p$ seguirà la stessa cadenza.\\ 
...
\begin{itemize}
  ...
  \[\mathbf{G}(r_0\rightarrow \mathbf{X}\neg r_0)\land\mathbf{G}(\neg
  ...
  \item quando $r_0$ passa da 1 a 0 il registro $r_1$ cambia il proprio valore
...
\begin{itemize}
  ...
  \[\mathbf{G}(g\rightarrow\mathbf{X}(g\,\mathbf{U}\, r))\]
...
\begin{center}
  ...
  $(M,t)\Vdash B$, altrimenti contiene 0.  
\end{center}
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item \textbf{iterazione:}
    \begin{itemize}
      \item $\forall\,t\geq 0$ se $Tab[\mathbf{F}\,C, t] = 0$ allora assegna
  ...
  \begin{enumerate}
    ...
    \item \textbf{iterazione:}
    \begin{itemize}
      \item $\forall\,t\geq 0$ se $Tab[C\,\mathbf{U}\,D, t] = 0$ e\\
      $Tab[C,t]=1$ allora assegna a $Tab[C\,\mathbf{U}\,D, t]$ il valore
  ...
  Tab[D, t]$  
  \item se $B$ è del tipo $C \lor D$, $Tab[C \lor D, t] = 1$ se $Tab[C, t] = 1$
  o $Tab[D, t] = 1$, $Tab[C \lor D, t] = 0$ altrimenti
...
\begin{center}
  \begin{tikzpicture}[shorten >=1pt,node distance=3.8cm,on grid,auto]
    \node[] (q_1) {$\alpha_1=\{a,k1,k2,x,p\}$};
    \node[] (q_4) [below right=of q_1] {$\alpha_4=\{a,k1,y,p\}$};
    \node[] (q_2) [below left=of q_1] {$\alpha_2=\{b,k2,x,p\}$};
...
e 4 transizioni:
\begin{itemize}
  \item $E_X$, che rappresenta l’erogazione del prestito. $E_X $ ha $B$ e $M_X$
