...
\begin{table}[H]
  ...
  \begin{tabular}{c||c|c}
    ...
    $\square$ & $\frac{S,T(\square A)}{S,TA}T\square$&
                        $\frac{S,F(\square A)}{S_\square,A}F\square$\\
    \hline
  \end{tabular}
\end{table}
\noindent
...
$T\square$ posso ripetere $T(\square A)$, ricordando che il $T\,A$ ottenuto
...
\begin{esempio}
  ...
  $F\neg A$, avendo ripetuto nel calcolo la formula del $T\square$
  (anche se, per comodità, solo la prima volta che viene usata e non anche la
  seconda). 
\end{esempio}
