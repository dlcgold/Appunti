...
$$v_m=\frac{\Delta \vec{x}}{\Delta t}=\frac{x_2-x_1}{t_2-t_1}$$
...
$$v=\lim_{\Delta t \to 0}\frac{\Delta \vec{x}}{\Delta t}=\frac{d x(t)}{d t}$$
...
\end{center}
\textbf{DA QUI IN POI NON ANCORA FATTO A LEZIONE}\\
Se \textit{v} è costante si parla di \textit{Moto Rettilineo Uniforme}. Nota quindi l'equazione del moto $x(t)$ possiamo ricavare $v(t)$ derivando.\\
Si può procedere anche al calcolo di $x(t)$ avendo nota $v(t)$. Sappiamo che lo spostamento totale è: $\Delta \vec{x}=\sum_{i=1}^n \Delta \vec{x}_i=\sum_{i=1}^n v_{m_i} \Delta t$ e che, per intervalli infinitesimi $dx=v(t) dt$. Si ha quindi:
$$\Delta x=\underbrace{\int_{x_0}^x dx}_{x(t)-x_0}=\int_{t_0}^t v(t) dt$$
$$\downarrow$$
