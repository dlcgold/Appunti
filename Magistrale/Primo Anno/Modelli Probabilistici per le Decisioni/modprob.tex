...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    collegano $V$ al cammino solo \textbf{tail to tail} (ovvero da $V$ escono i
    due archi: $\to V\gets$) e quindi il cammino è bloccato da $V$
    \item lungo il cammino si ha una variabile $V\in E$ (quindi di cui conosco
    il valore) e gli archi che
    collegano $V$ al cammino solo \textbf{tail to head} (ovvero un arco entra in
    ...
    collegano $V$ al cammino solo \textbf{head to head}
  \end{itemize}
\end{definizione}
...
\textbf{Esempi su slide. Su elearning file sulla \textit{Moralizzaizone},
  argomento extra}.
\end{document}  
