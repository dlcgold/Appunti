...
\begin{definizione}
  ...
  \begin{enumerate}
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}[node distance=2cm,bend angle=45,auto]
        \node [place, tokens = 1] (p0)  {};
        \node [transition] (t1) [above right of = p0, label=above:$e_1$] {};
        \node [transition] (t2) [below right of = p0, label=below:$e_2$] {};
        \node [place] (p1) [right  of = t1] {};
        \node [place] (p2) [right of = t2] {};
        {}; 
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}[node distance=2cm,bend angle=45,auto]
        ...
        \node [place, tokens = 1] (p00) [below of = p0] {};
        \node [transition] (t1) [right of = p0, label=above:$e_1$] {};
        \node [transition] (t2) [right of = p00, label=below:$e_2$] {};
        \node [place] (p1) [right  of = t1] {};
        {}; 
...
portassero dei secchi per andare a spegnere un fuoco. Se due omini si incontrano
lungo la strada si scambiano 
il secchio. Ma a Petri non va bene questo modello avendo a che fare col il
tempo, non discreto. Cerca quindi un modello combinatorio in cui si hanno varie
configurazioni che possono essere osservate. Gli omini potrebbero essere quindi
...
\begin{figure}[H]
  \centering
  \includegraphics[scale = 0.5]{img/inc1.jpg} 
\end{figure}
Possiamo quindi produrre la rete in base alle condizioni:
\begin{figure}[H]
  \centering
  \includegraphics[scale = 0.5]{img/inc2.jpg} 
\end{figure}
Posso modellare i vari percorsi tramite linee colorate con quelle tratteggiate
in alto che rappresentano lo stato iniziale e quelle in basso quello finale.
\begin{figure}[H]
  \centering
  \includegraphics[scale = 0.5]{img/inc3.jpg} 
\end{figure}
\textbf{rivedere questa parte.}\\
\begin{definizione}
  Definiamo:
  \[N=(B,E,F)\]
  come una \textbf{rete causale}, ovvero una rete di occorrenze senza conflitti,
  sse:
  \begin{itemize}
    \item $\forall b\in B:|\,^{\bullet}b|\leq 1\land |b^{\bullet}|\leq 1$, ovvero
    non si hanno conflitti
    \item $\forall a,y\in B\cup E:(x,y)\in F^+\implies (y,x)\not\in F^+$, ovvero
    non si hanno cicli
    \item $\forall e\in E:\{x\in B\cupE|\,xF^*e\}$ è finito
  \end{itemize}
  la rete può essere infinita.
\end{definizione}
Ad una rete causale è possibile associare un ordine parziale:
\[(X,\leq)=(B\cuo E, F^*)\]
In tal caso possono non distinguere più condizioni ed eventi.\\
\begin{definizione}
  Data una rete causale $N=(B,E,F)$ e dato un ordine parziale $(X, \leq)$ con
  $X=B\cup E$ si ha che, presi $x,y\in X$ come elementi che occorrono nella
  storia di $x$ si hanno le seguenti diciture:
  \begin{itemize}
    \item $x\leq y$ corrisponde a \textbf{$x$ causa $y$}
    \item $x \mbox{ \textbf{li} }y$ indica che $x\leq y\lor y\leq x$ e quindi
    corrisponde a \textbf{$x$ e $y$ sono casualmente dipendenti}
    \item $x \mbox{ \textbf{co} }y$ indica che $\neg(x< y)\land \neg(y < x)$ e
    quindi corrisponde a \textbf{$x$ e $y$ sono casualmente indipendenti} 
  \end{itemize}
\end{definizione}
\begin{esempio}
  Presa la rete causale:
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=2cm,bend angle=45,auto]
...
%       \node [transition] (t2) [below left of = p0, label=left:p] {};
%       \node [place, tokens = 1] (p1) [below right of = t2, label=below:P1] {}; 
%       \node [place, tokens=2] (p3) [right of = t1, label=above:B] {};
%       \node [transition] (t4) [right of = p3, label=right:e] {};
