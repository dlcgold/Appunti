...
$M$ è quindi $M\subseteq \{0,1\}$.
\textbf{risentire parte}\\
...
\begin{definizione}
  ...
  \begin{itemize}
    ...
    \item $d(\vec{x},\vec{z})\leqd(\vec{x},\vec{y})+d(\vec{y},\vec{z})$,
...
\begin{definizione}
  ...
  \[w_j(\vex{x})\]
  Avendo che:
  \[w_j(\vex{x})=\sum_{i=1}^n x_1=d_h(\vec{x},\vec{0})\]
\end{definizione}
\begin{esempio}
  Prendiamo $n=3$. \\
...
\begin{itemize}
  ...
  1 da quella errata che però non è quella giusta, introducendo un nuovo errore
...
\end{document} 
% LocalWords:  clock burst Hamming codeword
