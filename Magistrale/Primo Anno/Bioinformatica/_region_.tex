...
\textbf{DNA} e \textbf{RNA}, intese come molecole biologiche che vengono
trattate come stringhe.\\
Il \textbf{DNA (\textit{acido desossiribonucleico})} è una molecola composta da
nucleotidi, che è formato da:
\begin{itemize}
  \item il desossiribosio, uno zucchero, \textbf{D}
  \item un gruppo fosfato, \textbf{p}
  \item una base azotata (Adenina, Citosina, Guanina, Timina). Citosina e Timina
  sono dette \textbf{piramidini}, le altre due \textbf{purine}
\end{itemize}
...
così le \textbf{due sequenze primarie appaiate}, una detta \textbf{forward
  strand (+)} e una detta \textbf{reverse strand (-)}.\\
...
% aggiungere tabella amminoacidi per sfizio
...
c'entrano con gli altri. L'uomo ha circa 20000 geni codificanti, ciascuno che
...
\begin{itemize}
  ...
  originale 
\end{itemize}
Per il sequenziamento si è usato in primis il metodo Sanger, con read lunghe
circa 1000bp ma molto costose è la \textbf{first generation}.\\
Si hanno poi la \textbf{Next-generation sequencing} di seconda generazione con
read 
tra i 100 e i 400bp con alta copertura e poco costo.\\
Si hanno poi la \textbf{Next Next generation sequencing} con read di 10000bp ma
...
% LocalWords:  superstringa sottostringa

\message{ !name(bio.tex) !offset(-998) }
