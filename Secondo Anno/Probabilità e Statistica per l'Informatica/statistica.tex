...
preferisce considerare alternativamente alla varianza campionaria, una nuova
statistica detta \textbf{varianza campionaria n-sima corretta}.\\
Nel caso particolare in cui la popolazione sia normalmente distribuita si possono
dimostrare due importanti risultati che coinvolgono la statistica $S_n^2$ e a tale scopo si introduvcono due nuove variabili aleatorie:
\[Q_{n}=\frac{n \cdot S_{n}^{2}}{\sigma^{2}}=\frac{(n-1) \cdot \hat{S}_{n}^{2}}{\sigma^{2}}\]
\[T_{n}=\frac{\overline{X}_{n}-\mu}{\frac{\hat{S}_{n} }{\sqrt{n}}}\]
con $\hat{S}_{n}=\sqrt{\hat{S}_{n}^2}$
Queste due variabili sono dette ``non statistiche'' in quanto esse sono
funzioni anche dei parametri $\mu$ e $\sigma^2$, che vanno pensati come non noti.\\
Si dimostra che $Q_n$ è distribuito come una \textbf{Chi-quadro} $Q_n\sim \chi^2_{n-1}$ e $T_n$ come una \textbf{t di Student} $T_n\sim t_{n-1}}$, entrambi con $nb-1$ gradi di libertà. Le distribuzioni delle quantità $Q_n$ e $T_n$  non dipendono da $\mu$ e $\sigma^2$ poiché la \textbf{Chi-quadro} e la \textbf{t di Student} hanno i gradi di
libertà $ n-1$ quale unico parametro.\\
...
Per stimare un generico parametro $\vartheta$ possono essere definiti diversi stimatori e spesso si stabilisce un ordine di precedenza, con stimatorin preferibili ad altri.
pagina 385
\end{document}
