...
per cui la gestione efficiente ed ottimale dei dati è fondamentale.\newline
...
nei sistemi informatici dai \emph{dati}, la cui differenza tra di loro è che i dati sono valori senza alcun valore 
mentre le informazioni sono dei dati interpretati, a cui si attribuisce una semantica.
...
\begin{itemize}
        \item \textbf{DLL(Data Definition Languages)} linguaggio per definire i dati
...
\end{itemize}
Noi vedremo SQL per definire i database, linguaggio basato sull'algebra lineare che implementa tutti e 4 le tipologie 
...
La progettazione di un database, prevede la definizione e l'esecuzione delle seguenti fasi:
\begin{itemize}
...
\begin{itemize}
    \item entità: classe di oggetti con proprietà comune ed esistenza "autonoma", della quale si vogliono specificare 
        fatti specifici;ogni entità ha un nome univoco, espressivo e al singolare.
    \item relazione: rappresentano legami logici, significativi per la realtà da modellare, tra due o più entità.\newline
    ...
    \item attributo semplice: associa ad ogni istanza di entità o associazione un valore, definito su un dominio di valori,
    ...
    \item attributo composto: raggruppamento di attributi di una medesima entità/relazione con affinità di significato e/o uso
    ...
    \item cardinalità delle relazioni: vengono specificate per ogni relazione e descrivono il numero minimo e massimo di occorrenze
    ...
    \item cardinalità di un attributo: descrivono il numero minimo e massimo di valori dell'attributo 
    ...
    \item identificatore interno: permette di identificare in maniera univoca un'entità ed un identificatore è interno
          in caso sia uno o più attributi di un'entità, tutti con cardinalità $(1, 1)$.
    \item identificatore esterno: un identificatore è esterno, in caso un'entità $E$  viene identificata da un'attributo
    ...
    \item generalizzazione: rappresentano legami logici tra un entità $E$, detta padre, e una serie di entità $E_1, E_2, \dots, E_n$,
          ...
          \end{itemize}
    \item sottoinsieme: generalizzazione con soltanto un entità figlia, di cui solitamente rappresenta una parte 
...
\end{center}
Un'occorrenza, o istanza, di un'entità, è un oggetto della classe che l'entità rappresenta, ma noi rappresentiamo
le entità dello schema concettuale non le singole istanze: si parla di \textit{ conoscenza astratta} se si parla di entità
e di \textit{conoscenza concreta} se si parla di un'istanza di un'entità.
affinità nel loro significato o uso (per esempio per un indirizzo si ha via, comune, cap etc...). 
Nessuno impone un tipo per un certo attributo, possono essere anche complessi di cui però non ci interessano le informazioni che lo rappresentano (per esempio una foto può essere un attributo, ma se ho bisogno, per esempio, dell'autore della foto, non sarà più un attributo ma un'altra entità). 
