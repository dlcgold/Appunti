...
Che nuovo calcolo proviene da un'idea di Dyckhoff, che aveva ragionato sulla
...
il lavoro di Dyckhoff, hanno cercati di applicare le stesse regole al calcolo a
...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
Si noti come nel caso universale bisogna dividere il tableaux di due, avendo un
...
risultato teorico, il \textbf{lemma di Lindembuam} (\textit{potrebbe essere
  errato il nome del lemma}), che dice che non si ottiene mai il tableaux
...
\begin{esempio}
  ...
  si cancella tutto e avendo che sicuro non chiuderà mai il tableaux.\\
...
\begin{esempio}
  ...
  \begin{gather*}
    ...
    \frac{F_C A(a), TA(a)}{}
  \end{gather*}
  Avendo che il tableaux chiude.\\
  Si può vedere che altre scelte avrebbero portato il tableaux a non chiudere
...
\begin{gather*}
  ...
  F_C A(a), TA(a)
\end{gather*}
e quindi il tableaux chiude ma Miglioli e Moscato hanno notato che sostituire il
...
Questa regola farebbe chiudere il tableaux per la formula del teorema ma anche
...
chiudere il tableaux, comportando un calcolo molto difficile da usare. Infatti
in aggiunta alle regole di backtracking abbiamo anche la possibilità di
applicare tale regola in qualsiasi punto del tableaux, non scaturendo dai
...
\begin{esempio}
  ...
  \begin{gather*}
    ...
    TA(a), F_CA(a)
  \end{gather*}
  Avendo che il tableaux chiude e questo è anche possibile grazie al calcolo con
  il segno $F_C$. Con solo il calcolo alla Fitting non si sarebbe comunque
  chiuso il tableaux in maniera così ``pulita'', in quanto si sarebbe dovuto
...
\begin{esempio}
  ...
  \begin{gather*}
    ...
    F\neg A
  \end{gather*}
  Avendo che il tableaux non chiude. So però che la formula è dimostrabile in
...
\begin{esempio}
  ...
  \begin{gather*}
    ...
    T\neg A, T\neg\square\neg A, F\neg A
  \end{gather*}
  avendo che il tableaux chiude, per la coppia complementare $T\neg A$ e
...
tableaux (e abbiamo visto che la soluzione \textit{naive} al problema è fare la
...
presente in ogni riga del tableaux. Si introduce quindi un segno che
...
In termini di chiusura del tableaux $T_C$ si comporta come $T$.\\
...
\begin{esempio}
  ...
  \begin{gather*}
    ...
    T_C\neg A,F_\neg A
  \end{gather*}
  Avendo che il tableaux chiude, per di più anche senza la ripetizione del
  $T_C\neg$. 
\end{esempio}
\noindent
...
\[\frac{S}{S_C}TH,\,\,\,S_C\neq \emptyset\]
Questa regola è non
deterministica e posso applicarla in qualsiasi punto del tableaux, al più di non
...
un calcolo a tableaux con un calcolo ad assiomi alla Hilbert (anche perché
...
può essere applicata in qualsiasi punto del tableaux in vista dell'applicazione
...
più piccolo, in caso di molteplicità di regole da poter applicare, come visto in
figura \ref{fig:pitp}.\\
...
\textbf{Su slide dettagli delle specifiche del software di benchmark ILTP e
...
\begin{figure}
  ...
  \caption{Flowchart di PITP. Si noti che $Opt_2$ è eseguito con $C_2$ e $Opt_3$
    ...
    $\tau = \tau^{-1}$. Se il tableaux chiude la formula è dimostrabile.}
  \label{fig:pitp}
\end{figure}
