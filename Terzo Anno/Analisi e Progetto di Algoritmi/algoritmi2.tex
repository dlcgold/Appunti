...
\begin{shaded}
  \begin{algorithm}
    ...
    \Else
    \State \textbf{return} $\max(v_i+R-OPT(p[i]), R-OPT(i-1))$
    \EndIf
    \EndFunction
  \end{algorithm}  
\end{shaded}
...
\begin{shaded}
  \begin{algorithm}
    ...
    \State $M[i]\gets 0$
    \Else
    \State $M[i]\gets\max(v_i+AR-OPT(p[i]), AR-OPT(i-1))$
    \EndIf
    \EndIf
...
algortimo ma bisogna sommare $\Theta(n\log n)$, in quanto bisogna
ordinare le attività sul tempo di fine.  
...
\begin{shaded}
  \begin{algorithm}
    ...
    \State $max \gets 0$
    \For{$j\gets 1$ \textbf{to} $i-1$}
    \If {$a_j<a_i$ \textbf{AND} $b_j<b_i$ \textbf{AND} $c_j<c_i$
    \textbf{AND} $z[j]<max$ }
    \State $max\gets z[j]$
    \EndIf
    \EndFor
