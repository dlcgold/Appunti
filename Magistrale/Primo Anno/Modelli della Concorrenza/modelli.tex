...
\begin{itemize}
  \item \textit{Nil} o $0$ che ha un solo stato senza transizioni
  \begin{center}
    \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
      \node[state] (q_0) {$Nil$};
    \end{tikzpicture}
  \end{center}
  \item \textit{prefisso}, in cui si ha $\alpha\cdot p,\,\alpha\in Act,\,p\in
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \item \textit{Somma}, $p_1+p_2,\,\,p_1,p_2\in processi$, con la regola di
  ...
  \[oppure\]
   ...
   \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \item posso avere $\sum_{i\in I}p_i$ avendo multiple somme di processi:
  ...
  \begin{center}
    \begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
      \node[state] (q_0) {$a\cdot p_1'|\overline{a}\cdot p_2'$};
      \node[state] (q_1) [below right=of q_0] {$\overline{a}\cdot p_1'|p_2'$};
      \node[state] (q_3) [below left=of q_0] {$p_1'|\overline{a}\cdot p_2'$};
      \node[state] (q_2) [below right =of q_3] {$p_1'|p_2'$};
      \path[->]
  ...
  \[\frac{p\stackrel{\alpha}{\rightarrow}p'}{P_{\backslash L}
    ...
    ,\,\alpha,\overline{\alpha}\in L,\,\,L\subseteq A\]
...
\end{itemize}
Quindi dato un CCS posso associare un LTS.
\end{document}
...
% LocalWords:  bisimulazione sender ossrvabile  Nil rietichettatura
% LocalWords:  rinominazione
