...
\begin{esempio}
  Definiamo una funzione in C che riceve un intero (la lunghezza del vettore) e
  un vettore e restituisce un numero intero.
  \begin{listing}[ht]
    \begin{minted}{c}
    ...
    \label{listing:1}
  \end{listing}
  Iniziamo a chiederci cosa fa la funzione e scopriamo che cerca il massimo in
  un vettore. Si può cercare una dimostrazione del codice.\\
  Ricostruiamo la strategia della funzione, ovvero spostarsi lungo il vettore e
  ...
  Al termine di una certa iterazione $x$ contiene il massimo contiene il massimo
...
\[
  \begin{cases}
    v[0]\leq x \land v[1]\leq x\land\ldots v[n-1]\leq\\
...
succedere che una serie di istruzioni non facciano terminare il programma quindi
in realtà quanto detto sopra è un'approssimazione della realtà.
\begin{definizione}
  ...
  \begin{itemize}
    ...
    \item $P$ è un ``programma'' (anche un frammento o una sola istruzione) che
...
Hoare sono triple come $(\alpha, P,\beta)$ quindi è una logica di tipo diverso
anche se si appoggia sulla logica proposizionale.
...
soddisfi la precondizione ma che eseguendo non si avveri la postcondizione).
...
\begin{definizione}
  ...
  premesse e una delle due non è una tripla di Hoare ma un'implicazione della
 ...
 \begin{definizione}
   ...
   Dove con $p[E/x]$, come precondizione, indichiamo una \textbf{sostituzione} e
   indica che cerchiamo in $p$ tutte le occorrenze $x$ e le sostituiamo con $E$.
   \begin{esempio}
     ...
     che a $x$ assegno $y+1$ posso essere sicuro che anche $x$ sia positivo).
   \end{esempio}
   \begin{esempio}
   ...
   \end{esempio}
 \end{definizione}
 Per proseguire bisogna definire il concetto di \textbf{invariante}.
 \begin{definizione}
   La quinta regola è quella relativa all'iterazione
 \end{definizione}
\end{document}
