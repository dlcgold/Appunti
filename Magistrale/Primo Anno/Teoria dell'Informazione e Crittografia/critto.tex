...
%12.10 parte 2 lezione 3
\begin{esempio}
  Se devo spedire 24 bit posso rappresentarli in vari modi, secondo vari
  rettangoli:  
  \begin{itemize}
    \item una riga per 24 colonne (più la riga di controllo), con quindi 24
    check digit
...
possibili, avendo meno cifre di controllo e migliorando la ridondanza.\\
La ridondanza la posso vedere come una funzione:
\[f(n,m)\]
e si può cercare il punto minimo, che è in $m=n$.\\
Si ha quindi, avendo un quadrato:
\[R_\square=\mbox{manca una parte}=1+\frac{2}{(n-1)}+\frac{1}{(n-1)^2}\]
\subsection{Codici triangolari}
Vediamo quindi i \textbf{codici a correzione triangolari.}\\
In questo caso si ha un triangolo, ad esempio (con $n=4$):
\begin{table}[H]
  \centering
  \begin{tabular}{ccccc}
