...
\begin{itemize}
  \item $\vdash_{INT}(A\to\neg B)\to(B\to \neg A)$
  \item $\vdash_{INT}(\neg A\lor\neg B)\to(B\to \neg A)$
  \item $\vdash_{INT}(A\to B)\iff(\neg B\to \neg A)$, detta \textbf{legge di
    contrapposizione} (che ricorda il teorema di deduzione della logica classica
  che legava dimostrabilità e implicazione, avendo a livello proposizionale che
  se con ipotesi $A$ dimostro $B$, $A\vdash B$ allora solo allora $A\to B)
  \item $\vdash_{INT}((((A\to B)\to A)\to A)\to B)\to B$ (\textit{di questa non
    c'è la soluzione nella lezione})
\end{itemize}
...
dall'alto, nel seocndo dal basso.\\
...
% LocalWords:  l'istanziazione Kripke contromodello seocndo
