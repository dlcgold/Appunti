...
\begin{enumerate}
  ...
  accedere ai dati utili dell'applicazione
  ...
  e si occupa dell'interazione con utenti e agenti 
...
Dal punto di vista del \textbf{Controller design} si ha:
\begin{itemize}
  \item \textbf{Page Controller}, che si occupa di controllare i parametri delle
  richieste e richiamare la business logic, di selezionare la vista successiva
  ...
  \begin{enumerate}
    ...
    \item il Page Controller decide la vista successiva
    \item il Page Controller prepara i dati da mostrare nella vista successiva
    \item il Page Controller porta il controllo (e i dati) alla vista
  \end{enumerate}
  ...
  esegue la specifica operazione che è stata richiesta, passando il parametro
  come parametro. Vediamo quindi l'UML associato:
  \begin{figure}[H]
    \centering
    \includegraphics[scale = 0.7]{img/mvc2.pdf}
  \end{figure}
  anche nel caso di un front controller si potrebbero avere più controller.\\
  Nel dettaglio l'\textit{handler}:
  \begin{itemize}
    \item riceve la richiesta dal server 
    \item esegue operazioni generali/comuni 
    \item decide l'operazione che deve essere eseguita 
  ...
  duplicaizone del codice tra i vari Controller, permette una semplice
  configurazione del server avendo una sola servlet, permette di gestire
  dinamicamente dinamicamente nuovi comandi e facilita l'estensione del
  Controller (avendo solo un controller)
...
\end{itemize}
\newpage
Passiamo quindi al \textbf{design view}, dove si hanno:
\begin{itemize}
  ...
  output.\\
  Purtroppo è difficile includere la logica di implementazione nella vista 
  ...
  sarebbe più intuitivo e facile da implementare
  ...
  \end{itemize}
\end{itemize}
Si ha \textit{STRUTS2} che è un framework con l'implementazione integrata di più
design pattern:
\begin{itemize}
  \item MVC2
...
\end{itemize}
Si ha un file \texttt{web.xml} per permettere s STRUTS2 di catturare le
richieste, indipendentemente dall'applicazione. Questo framework ha anche
versioni GUI integrabili con editor come Ecplipse.
\end{document}  
